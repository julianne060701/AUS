...
<div class="modal fade" id="assignmentModal" tabindex="-1" role="dialog" aria-labelledby="assignmentModalLabel" aria-hidden="true">
    ...
    <div class="form-group">
        ...
        <select class="form-control" id="installer_name" name="installer_name" required>
            ...
            <?php
            // Fetch users with role 'installer'
            $installers_query = "SELECT id, username, full_name FROM users WHERE role = 'installer' ORDER BY full_name ASC";
            $installers_result = mysqli_query($conn, $installers_query);
            
            if ($installers_result && mysqli_num_rows($installers_result) > 0) {
                while ($installer = mysqli_fetch_assoc($installers_result)) {
                    // Use full_name for both value and display
                    $installer_name = !empty($installer['full_name']) ? $installer['full_name'] : $installer['username'];
                    echo "<option value='" . htmlspecialchars($installer_name) . "'>" . htmlspecialchars($installer_name) . "</option>";
                }
            } else {
                echo "<option value='' disabled>No installers available</option>";
            }
            ?>
        </select>
    </div>
...
<div class="modal fade" id="editScheduleModal" tabindex="-1" role="dialog" aria-labelledby="editScheduleModalLabel" aria-hidden="true">
    ...
    <div class="form-group">
        ...
        <select class="form-control" id="edit_installer_name" name="installer_name" required>
            <option value="">Select Installer</option>
            <?php
            // Fetch users with role 'installer' for edit form
            $edit_installers_query = "SELECT id, username, full_name FROM users WHERE role = 'installer' ORDER BY full_name ASC";
            $edit_installers_result = mysqli_query($conn, $edit_installers_query);
            
            if ($edit_installers_result && mysqli_num_rows($edit_installers_result) > 0) {
                while ($installer = mysqli_fetch_assoc($edit_installers_result)) {
                    // Use full_name for both value and display
                    $installer_name = !empty($installer['full_name']) ? $installer['full_name'] : $installer['username'];
                    echo "<option value='" . htmlspecialchars($installer_name) . "'>" . htmlspecialchars($installer_name) . "</option>";
                }
